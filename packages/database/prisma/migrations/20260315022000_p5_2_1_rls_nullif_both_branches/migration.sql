-- ════════════════════════════════════════════════════════════
-- P5-2.1c: RLS NULLIF IN BOTH OR BRANCHES
-- ════════════════════════════════════════════════════════════
-- Bug: PostgreSQL OR short-circuit GARANTI DEĞİL.
-- Önceki migration'da ikinci branch:
--   "tenantId" = current_setting(...)::uuid
-- evaluating ''::uuid → invalid input syntax for type uuid.
--
-- Fix: İkinci branch'te de NULLIF kullan:
--   "tenantId" = NULLIF(current_setting(...), '')::uuid
-- NULLIF('','') → NULL, NULL::uuid → NULL, tenantId = NULL → false
-- → OR true ile birleşince → true (passthrough korunur)
-- ════════════════════════════════════════════════════════════
