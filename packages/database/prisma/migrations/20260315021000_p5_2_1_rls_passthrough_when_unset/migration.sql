-- ════════════════════════════════════════════════════════════
-- P5-2.1b: RLS PASSTHROUGH WHEN app.tenant_id UNSET
-- ════════════════════════════════════════════════════════════
-- Önceki migration (20260315020000) NULLIF ile boş string crash'ini
-- düzeltti ama tüm satırları blokladı (uuid = NULL → false).
--
-- Interceptor artık set_config YAPMIYOR (filter-only):
--   - Katman 1 (WHERE filter): her sorguya tenantId enjekte eder
--   - Katman 2 (RLS): $tenantTransaction ile set_config yapıldığında
--     ek güvence sağlar
--
-- Bu migration: RLS policy'yi "passthrough when unset" yapar:
--   - app.tenant_id boş/NULL → tüm satırlara izin ver (WHERE filtre korur)
--   - app.tenant_id set → strict tenant isolation (çift katman)
--
-- WITH CHECK: yazma işlemlerinde her zaman set_config zorunlu.
-- Interceptor create/update/delete'e tenantId enjekte eder,
-- ancak doğrudan SQL yazma set_config gerektirir.
-- ════════════════════════════════════════════════════════════
