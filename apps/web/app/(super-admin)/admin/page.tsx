'use client';

import { Loader2, AlertCircle, Building2, Users, Calendar, TrendingUp } from 'lucide-react';
import { useAdminOverview, useGrowthMetrics, useHealthStatus, useVersionInfo } from '@/hooks/api/use-admin';

export default function AdminOverviewPage() {
  const { data: overview, isLoading: ovLoading, error: ovError } = useAdminOverview();
  const { data: growth, isLoading: grLoading, error: grError } = useGrowthMetrics();
  const { data: health } = useHealthStatus();
  const { data: version } = useVersionInfo();

  const loading = ovLoading || grLoading;
  const error = ovError || grError;

  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight">Platform Genel Bakış</h1>
        {health && (
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${health.checks?.database?.status === 'up' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'}`}>
            {health.status === 'ready' ? '● Sağlıklı' : '○ Sorunlu'}
          </span>
        )}
      </div>

      {error ? (
        <div className="flex items-center gap-2 text-destructive py-8 justify-center">
          <AlertCircle className="h-5 w-5" />
          <span>{error.message.includes('401') ? 'Geçersiz API anahtarı.' : 'Veriler yüklenemedi.'}</span>
        </div>
      ) : loading ? (
        <div className="flex justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
      ) : (
        <>
          {/* Stat cards */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={Building2} label="Toplam Salon" value={overview?.totalTenants ?? growth?.totalSalons ?? 0} />
            <StatCard icon={Users} label="Aktif Salon" value={overview?.activeTenants ?? growth?.activeSalons ?? 0} accent />
            <StatCard icon={Calendar} label="Bugünkü Randevu" value={growth?.bookingsToday ?? 0} />
            <StatCard icon={TrendingUp} label="Bu Ay Yeni" value={growth?.newSalonsThisMonth ?? 0} />
          </div>

          {/* Health + Version */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border p-4 space-y-2">
              <h3 className="text-sm font-semibold">Platform Sağlığı</h3>
              {health?.checks ? (
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">Database</span><span className={health.checks.database.status === 'up' ? 'text-emerald-600' : 'text-red-600'}>{health.checks.database.status} ({health.checks.database.latencyMs}ms)</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Redis</span><span className={health.checks.redis.status === 'up' ? 'text-emerald-600' : 'text-red-600'}>{health.checks.redis.status} ({health.checks.redis.latencyMs}ms)</span></div>
                </div>
              ) : <p className="text-sm text-muted-foreground">Veri yok</p>}
            </div>
            <div className="bg-white rounded-xl border p-4 space-y-2">
              <h3 className="text-sm font-semibold">Versiyon</h3>
              {version ? (
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">Version</span><span>{version.version}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Node</span><span>{version.nodeVersion}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Ortam</span><span>{version.environment}</span></div>
                </div>
              ) : <p className="text-sm text-muted-foreground">Veri yok</p>}
            </div>
          </div>

          {/* Plan distribution */}
          {growth?.planDistribution && Object.keys(growth.planDistribution).length > 0 && (
            <div className="bg-white rounded-xl border p-4 space-y-2">
              <h3 className="text-sm font-semibold">Plan Dağılımı</h3>
              <div className="flex gap-4">
                {Object.entries(growth.planDistribution).map(([plan, count]) => (
                  <div key={plan} className="text-center">
                    <p className="text-lg font-bold">{count as number}</p>
                    <p className="text-xs text-muted-foreground">{plan}</p>
                  </div>
                ))}
              </div>
            </div>
          )}
        </>
      )}
    </div>
  );
}

function StatCard({ icon: Icon, label, value, accent }: { icon: React.ComponentType<{ className?: string }>; label: string; value: number; accent?: boolean }) {
  return (
    <div className="bg-white rounded-xl border p-4 flex items-center gap-3">
      <div className={`p-2 rounded-lg ${accent ? 'bg-primary/10' : 'bg-muted'}`}>
        <Icon className={`h-5 w-5 ${accent ? 'text-primary' : 'text-muted-foreground'}`} />
      </div>
      <div>
        <p className="text-2xl font-bold">{value}</p>
        <p className="text-xs text-muted-foreground">{label}</p>
      </div>
    </div>
  );
}
