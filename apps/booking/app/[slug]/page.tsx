/**
 * SALON BOOKING PAGE — Faz 16
 * ──────────────────────────────────────────────────────────────────────────────
 * Route: /[slug]  (ör: /luna-beauty)
 * Server Component — SEO optimizasyonu, SSR, Edge cache uyumlu.
 *
 * Bölümler:
 *   1. Hero      — Salon adı, rating, şehir, CTA
 *   2. Services  — Hizmet kartları
 *   3. Staff     — Personel kartları
 *   4. Booking   — Multi-step booking widget (Client Component)
 *   5. Location  — Adres + Google Maps embed
 *   6. Footer    — "Powered by Auralis" büyüme elementi
 */
...
import { fetchSalon, fetchServices, fetchStaff } from '@lib/api';
import { formatPrice, formatDuration }           from '@lib/utils';
import { BookingWidget }                         from '@components/BookingWidget';

// ── Metadata üreteci (SSR — Google indexlenebilir) ────────────────────────────
...
export async function generateMetadata({
  ...
  if (!salon) {
    return { title: 'Salon Bulunamadı | Auralis' };
  }
...
export default async function SalonPage({
  ...
  // Salon bilgisini al (404 kontrolü için önce)
  const salon = await fetchSalon(slug);
  if (!salon) notFound();
